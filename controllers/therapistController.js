// controllers/therapistController.js (VERSION FINAL Y ESTABLE)

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient(); 
const jwt = require('jsonwebtoken');

// Función helper para generar el token JWT (si la usas en tu ruta de auth)
...
exports.getPatientProfile = async (req, res) => {
    ...
    try {
        // Asegura que el terapeuta solo pueda ver a sus propios pacientes
        const patient = await prisma.user.findUnique({
            where: { 
                id: patientId,
                therapistId: therapistId, // CRÍTICO: Comprobación de propiedad
                role: 'PATIENT'
            },
            select: { 
                id: true, 
...
exports.createGoal = async (req, res) => {
    ...
    try {
        // 1. Verificar si el paciente está asignado a este terapeuta
        const patient = await prisma.user.findUnique({
        ...
        const newGoal = await prisma.goal.create({
            data: {
                patientId: patientId,
                therapistId: therapistId, // Para rastrear quién la asignó
                name,
                description,
...
exports.getPatientGoals = async (req, res) => {
    ...
    try {
        // CRÍTICO: Asegura que el paciente esté asignado antes de obtener las metas
        const patient = await prisma.user.findUnique({
             where: { id: patientId, therapistId: therapistId, role: 'PATIENT' }
        });
...
exports.updateGoal = async (req, res) => {
    ...
    try {
        const updatedGoal = await prisma.goal.update({
            where: { 
                id: goalId,
                therapistId: therapistId // Solo el creador puede actualizarla
            },
            data: updateData,
        });
